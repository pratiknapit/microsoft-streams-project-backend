...
def test_invalid_token(clear, channel1, user2):
    with pytest.raises(AccessError):
        assert channel_remove_owner_v2("random_token", channel1['channel_id'], user2['auth_user_id'])

def test_invalid_user(clear, user1, channel1, user2):
    with pytest.raises(InputError):
        assert channel_remove_owner_v2(user1['token'], channel1['channel_id'], -1)

def test_invalid_channel_id(clear, user1, user2):
    with pytest.raises(InputError):
        assert channel_remove_owner_v2(user1['token'], -1, user2['auth_user_id'])

def test_non_member(clear, user1, user2, channel1, user3): 
    with pytest.raises(InputError):
        assert channel_remove_owner_v2(user1['token'], channel1['channel_id'], user3['auth_user_id'])

def test_only_owner(clear, user1, channel1):
    with pytest.raises(InputError):
        assert channel_remove_owner_v2(user1['token'], channel1['channel_id'], user1['auth_user_id'])
...
def test_non_global_member_non_owner(clear, user1, channel1, user2, user3):
    ...
    with pytest.raises(AccessError):
        assert channel_remove_owner_v2(user2['token'], channel1['channel_id'], user3['auth_user_id'])


def test_channel_remove_owner_works(clear, user1, user2, channel1):
    channel_join_v1(user2['token'], channel1['channel_id']) 
    ...
    channel_remove_owner_v2(user1['token'], channel1['channel_id'], user2['auth_user_id'])
    data_new = channel_details_v1(user1['token'], channel1['channel_id'])
